...
2. **`nvim_state` first, every time.** Before your first nvim-mcp tool call
   or file edit in a Neovim buffer each turn, call `nvim_state`. The user
   may have moved between turns — never rely on state from a previous turn.
...
4. **Context lines are prefixed with absolute line numbers** (e.g.
   `"28:   code here"`). Use them directly — don't guess from memory
   or read from the terminal display.
   In visual mode, a `selection` range marks highlighted lines.
