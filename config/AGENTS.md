...
2. **Always call `nvim_state` before any action that depends on cursor
   position, file, or mode.** The user may have moved between turns.
   Never rely on previously cached state for cursor-dependent operations.
